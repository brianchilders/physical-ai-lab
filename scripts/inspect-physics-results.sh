...
main() {
  ...
  docker run --rm --pull=never \
    --name physics-foundations-004-inspect \
    --gpus all \
    --user 1234:1234 \
    --entrypoint bash \
    -e ACCEPT_EULA=Y \
    -e PRIVACY_CONSENT=N \
    -e PHYSICS_FOUNDATIONS_RUNTIME_ROOT=/workspace/experiments/004-physics-foundations \
    -v "$repo_root_path:/workspace:ro" \
    -v "$(physics_foundations_output_dir):/workspace/experiments/004-physics-foundations/output:ro" \
    -v "$(physics_foundations_results_dir):/workspace/experiments/004-physics-foundations/results:rw" \
    ...
    -lc 'set -o pipefail; /isaac-sim/python.sh inspect_physics_results.py 2>&1 | tee /workspace/experiments/004-physics-foundations/results/inspect_physics_results.log; exit "${PIPESTATUS[0]}"'
