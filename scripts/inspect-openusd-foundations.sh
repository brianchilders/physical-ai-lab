...
main() {
  local repo_root_path runtime_root inspect_log
  ...
  docker run --rm --pull=never \
    --name openusd-foundations-002-inspect \
    --gpus all \
    --user 1234:1234 \
    --entrypoint bash \
    -e ACCEPT_EULA=Y \
    ...
    -lc 'set -o pipefail; /isaac-sim/python.sh inspect_scene.py 2>&1 | tee /workspace/experiments/002-openusd-foundations/results/inspect_scene.log; exit "${PIPESTATUS[0]}"'
